...
from model import CausalGCN, CausalGIN, CausalGAT, GINNet, GCNNet, GATNet, CausalGCN_share, CausalGIN_share, \
    CausalGAT_share, CausalGCN_begin, CausalGIN_begin, CausalGCN_CAL_M_D, CausalGCN_CAL_M, CausalGCN_CAL, CausalGCN_CAL_M_D_wo_season, \
    CausalGCN_CAL_M_Multiscale, CausalGCN_CAL_Multiscale
import numpy as np
import random
import torch
...
def parse_args():
    ...
    #################### 可调参数 #######################
    parser.add_argument('--model', type=str, default="CausalGCN_CAL_M_Multiscale", help="GCN, CausalGCN_CAL_M_D, CausalGCN_CAL_M, CausalGCN_CAL, CausalGCN_CAL_M_D_wo_season, CausalGCN_CAL_Multiscale, CausalGCN_CAL_M_Multiscale") # 可选模型对应文档中4个
    parser.add_argument('--data_name', type=str, default="ATSD_v4.3") # 用于保存模型/训练日志
    parser.add_argument('--node_num', type=int, default=85, help='number of metrics') # metrics个数
    parser.add_argument('--feature_dim', type=int, default=100, help='number of time steps') # 时间序列长度
    parser.add_argument('--hidden', type=int, default=256) # 隐藏层维度
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--c', type=float, default=0.5)
    parser.add_argument('--o', type=float, default=1.0)
    parser.add_argument('--co', type=float, default=0.5)
    parser.add_argument('--oc', type=float, default=0.5)
...
def get_model(args):
    ...
    def model_func11(num_features, num_classes):
        return CausalGIN_begin(num_features, num_classes, args)
    
    def model_func12(num_features, num_classes):
        return CausalGCN_CAL_M(num_features, num_classes, args)
    
    def model_func13(num_features, num_classes):
        return CausalGCN_CAL(num_features, num_classes, args)
    
    def model_func14(num_features, num_classes):
        return CausalGCN_CAL_M_D(num_features, num_classes, args)
    
    def model_func15(num_features, num_classes):
        return CausalGCN_CAL_M_D_wo_season(num_features, num_classes, args)
    
    def model_func16(num_features, num_classes):
        return CausalGCN_CAL_M_Multiscale(num_features, num_classes, args)
    
    def model_func17(num_features, num_classes):
        return CausalGCN_CAL_Multiscale(num_features, num_classes, args)
    ...
    elif args.model == "CausalGIN_begin":
        model_func = model_func11
    elif args.model == "CausalGCN_CAL_M":
        model_func = model_func12
    elif args.model == "CausalGCN_CAL":
        model_func = model_func13
    elif args.model == "CausalGCN_CAL_M_D":
        model_func = model_func14
    elif args.model == "CausalGCN_CAL_M_D_wo_season":
        model_func = model_func15
    elif args.model == "CausalGCN_CAL_M_Multiscale":
        model_func = model_func16
    elif args.model == "CausalGCN_CAL_Multiscale":
        model_func = model_func17
    else:
        assert False
    return model_func
